...
function ActivityEntry({ entry }) {
  ...
  return <details className="border-b last:border-b-0"><summary className="cursor-pointer list-none hover:bg-muted/40"><ActivityLine row={entry.rows[0]} headline={batch.headline} batch /><span className="sr-only">Show details</span></summary><div className="border-t bg-muted/20 px-4 py-2"><p className="mb-2 text-xs text-muted-foreground">Individual changes</p>{entry.rows.map((row) => <ActivityLine key={row.audit_id} row={row} nested />)}</div></details>
}
...
function ActivityLine({ row, headline, batch = false, nested = false }) {
  ...
  return <div className={`flex gap-3 p-4 ${nested ? 'border-t first:border-t-0' : 'border-b last:border-b-0'} sm:items-start`}><span className={`flex size-9 shrink-0 items-center justify-center rounded-full ${TONE_CLASSES[tone]}`}><Icon className="size-4" /></span><div className="min-w-0 flex-1"><p className="font-medium text-foreground">{headline || description.headline}</p>{!batch && <>{hasQuantity ? <p className="mt-1 flex flex-wrap items-center gap-2 text-sm text-muted-foreground"><span className="tabular-nums">{row.quantity_before == null ? '—' : formatStock(auditItem(row), row.quantity_before)} → {row.quantity_after == null ? '—' : formatStock(auditItem(row), row.quantity_after)}</span>{deltaLabel(row) && <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${Number(row.quantity_delta) > 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-destructive/10 text-destructive'}`}>{deltaLabel(row)}</span>}</p> : changes[0]?.text ? <p className="mt-1 text-sm text-muted-foreground">{changes[0].text}</p> : <p className="mt-1 text-sm text-muted-foreground">{ACTION_LABELS[row.action] || 'Updated'}</p>}{row.note?.trim() && <p className="mt-1 text-sm text-muted-foreground">Note: {row.note.trim()}</p>}{changes.length > 0 && <details className="mt-2"><summary className="cursor-pointer text-sm text-muted-foreground">View changes</summary><dl className="mt-2 grid gap-1 rounded-md bg-muted/60 p-3 text-sm">{changes.map((change, index) => <div key={`${change.label}-${index}`} className="grid gap-1 sm:grid-cols-[10rem_minmax(0,1fr)]"><dt className="font-medium text-foreground">{change.label}</dt><dd className="min-w-0 text-muted-foreground">{change.text || `${change.before} → ${change.after}`}</dd></div>)}</dl></details>}</>}</div><div className="shrink-0 text-left text-xs text-muted-foreground sm:text-right"><p title={formatManilaDateTime(row.created_at)}>{formatManilaTime(row.created_at)}</p><p className="mt-1">{formatRelative(row.created_at)}</p>{batch && <p className="mt-1 font-medium">Show details</p>}</div></div>
}
