...
export function InventoryItemDialog({ item, items, onSaved }) {
  ...
  return (
    <DialogContent size="md">
      <DialogHeader><DialogTitle>{editMode ? `Edit ${item.item_name}` : 'Add inventory item'}</DialogTitle></DialogHeader>
      <form onSubmit={save} className="flex min-h-0 flex-1 flex-col">
        <DialogBody className="grid gap-5">
          <div className="grid gap-1.5"><Label htmlFor="inventory-name">Name</Label><Input id="inventory-name" value={form.name} onChange={(event) => change('name', event.target.value)} /></div>
          <fieldset className="grid gap-2"><legend className="text-sm font-medium">Type</legend><div className="flex gap-2">{TYPE_OPTIONS.map((option) => <Button key={option.value} type="button" variant={form.itemType === option.value ? 'default' : 'outline'} className="min-h-11" disabled={editMode} onClick={() => changeType(option.value)}>{option.label}</Button>)}</div>{editMode && <p className="text-xs text-muted-foreground">Item type cannot be changed after creation.</p>}</fieldset>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {form.itemType === 'ingredient'
              ? <div className="grid gap-1.5"><Label htmlFor="inventory-unit">Unit</Label><Select value={form.trackingUnit} onValueChange={(value) => change('trackingUnit', value)} disabled={editMode} items={{ g: 'g', mL: 'mL', pc: 'pc' }}><SelectTrigger id="inventory-unit" className="w-full"><SelectValue /></SelectTrigger><SelectContent>{['g', 'mL', 'pc'].map((unit) => <SelectItem key={unit} value={unit}>{unit}</SelectItem>)}</SelectContent></Select></div>
              : <fieldset className="grid gap-2"><legend className="text-sm font-medium">Tracking unit</legend><div className="flex gap-2">{['pc', 'pkg'].map((unit) => <Button key={unit} type="button" variant={form.trackingUnit === unit ? 'default' : 'outline'} disabled={editMode} onClick={() => change('trackingUnit', unit)}>{unit}</Button>)}</div></fieldset>}
          </div>
      ...
      <DialogContent>
        ...
        <form onSubmit={save} className="flex min-h-0 flex-1 flex-col">
          <DialogBody className="grid gap-4">
            <p className="text-sm text-muted-foreground">Current quantity: {formatStock(item)}</p>
            <div className="flex flex-wrap gap-2">{['received', 'used', 'recount'].map((option) => <Button key={option} type="button" variant={mode === option ? 'default' : 'outline'} onClick={() => setMode(option)}>{option === 'received' ? 'Received' : option === 'used' ? 'Used' : 'Recount'}</Button>)}</div>
            <div className="grid gap-2"><Label htmlFor={`stock-amount-${item.inventory_item_id}`}>{mode === 'recount' ? 'Physical count' : 'Amount'}</Label><div className="flex gap-2"><NumberField id={`stock-amount-${item.inventory_item_id}`} value={amount} onValueChange={setAmount} min={0} suffix={enteredUnit} />{isPackTracked(item) && <div className="flex shrink-0 gap-1">{['pkg', 'pc'].map((unit) => <Button key={unit} type="button" variant={enteredUnit === unit ? 'default' : 'outline'} onClick={() => setEnteredUnit(unit)}>{unit}</Button>)}</div>}</div></div>
            <div className="grid gap-1.5"><Label htmlFor={`stock-note-${item.inventory_item_id}`}>Note (optional)</Label><textarea id={`stock-note-${item.inventory_item_id}`} className="min-h-20 w-full rounded-md border bg-transparent p-2.5 text-sm outline-none focus-visible:ring-3 focus-visible:ring-ring/50" maxLength={200} value={note} onChange={(event) => setNote(event.target.value)} /></div>
